...
static void bb_ui_draw_measures_right(UIState *s, int bb_x, int bb_y, int bb_w ) {
  ...
  if (UI_FEATURE_RIGHT_CPU_TEMP) {
    ...
    if(cpuList.size() > 0)
    {
      ...
      snprintf(uom_str, sizeof(uom_str), "");
      bb_h += bb_ui_draw_measure(s,  val_str, uom_str, "", bb_rx, bb_ry, bb_uom_dx, 
               val_color, lab_color, uom_color,  value_fontSize, label_fontSize, uom_fontSize );
      bb_ry = bb_y + bb_h;
  }
/*
...
static void bb_ui_draw_basic_info(UIState *s) {
    ...
    snprintf(str, sizeof(str), " %s (SR %.2f) (SC %.2f) (SD %.2f) (%d)",
    ...
    ui_draw_text(s, x, y, str, 29 * 2.5, COLOR_WHITE, "sans-semibold");

    snprintf(str, sizeof(str), " (A%.2f B%.2f C%.2f D%.2f %.2f)%s%s",
...
static void bb_ui_draw_UI(UIState *s) {
  ...
  const int bb_dmr_y = (box_y + (bdr_is * 1.5)) + 938;
// 스위치 화면 비우기 시작
#if UI_FEATURE_LEFT
  if(s->show_debug_ui)
...
static void ui_draw_tpms(UIState *s) {
  ...
  nvgTextAlign(s->vg, NVG_ALIGN_CENTER | NVG_ALIGN_BASELINE);
  const int pos_x = viz_tpms_x + 50;
  const int pos_y = 973;
  const int add_x = 80;
  const int add_y = 60;
  const int fontsize = 90;

  ui_draw_text(s, pos_x, pos_y + add_y, "", fontsize-20, COLOR_WHITE_ALPHA(200), "sans-regular");
  ...
  ui_draw_text(s, pos_x, pos_y - add_y, tpmsFl, fontsize, nvgRGBA(255, 255, 255, 255), "sans-semibold");
  ui_draw_text(s, pos_x + add_x, pos_y - add_y, tpmsFr, fontsize, nvgRGBA(255, 255, 255, 255), "sans-semibold");
  ui_draw_text(s, pos_x, pos_y, tpmsRl, fontsize, nvgRGBA(255, 255, 255, 255), "sans-semibold");
  ui_draw_text(s, pos_x + add_x, pos_y, tpmsRr, fontsize, nvgRGBA(255, 255, 255, 255), "sans-semibold");
}
