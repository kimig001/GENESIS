...
//  drawBrake(p);
  ...
  QString infoText;
  infoText.sprintf(" %s  SR(%.2f) SRC(%.2f) SAD(%.2f) SCC(%d)",
  ...
  p.drawText(rect().left() + 150, rect().height() - 70, infoText);

  infoText.sprintf(" A%.2f B%.2f C%.2f D%.2f/%.2f)",
  ...
  p.drawText(rect().left() + 265, rect().height() - 15, infoText);

  if ((int)(cpuTemp) >= 70) { QColor(255, 188, 3, 200); }
  if ((int)(cpuTemp) >= 80) { QColor(255, 0, 0, 200); }

  configFont(p, "Open Sans", 60, "Bold");
  infoText.sprintf("%.0f°C", cpuTemp);
  p.drawText(1700, 875, infoText);  
}
...
void NvgWindow::drawTpms(QPainter &p) {
  ...
  configFont(p, "Open Sans", 46, "Bold");

  QFontMetrics fm(p.font());
  QRect rcFont = fm.boundingRect("9");

  drawText2(p, 1680, 910, Qt::AlignRight, get_tpms_text(fl), get_tpms_color(fl));
  drawText2(p, 1803, 910, Qt::AlignLeft, get_tpms_text(fr), get_tpms_color(fr));
  drawText2(p, 1680, 965, Qt::AlignRight, get_tpms_text(rl), get_tpms_color(rl));
  drawText2(p, 1803, 965, Qt::AlignLeft, get_tpms_text(rr), get_tpms_color(rr));
}
